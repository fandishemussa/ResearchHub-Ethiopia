...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Import normalized AAU ETD harvested JSON records into ResearchHub PostgreSQL."
        )
    )
...
def parse_date(value: Any) -> date | None:
    ...
    try:
        parsed = datetime.fromisoformat(cleaned.replace("Z", "+00:00")).date()
...
def clean_keywords(
    ...
    *,
    ...
    return deduplicate_case_insensitive(cleaned_keywords)
...
def map_aau_record(
    ...
    if not title:
        raise ValueError("AAU record does not contain a valid title.")

    publication_date = parse_date(record.get("publication_date"))

    authors = deduplicate_case_insensitive(string_list(record.get("authors")))

    subjects = clean_keywords(string_list(record.get("subjects")))

    identifiers = deduplicate_case_insensitive(string_list(record.get("identifiers")))

    landing_page_url = record.get("landing_page_url")

    if isinstance(landing_page_url, str) and landing_page_url.strip():
        cleaned_landing_page_url = landing_page_url.strip()

        if cleaned_landing_page_url not in identifiers:
            identifiers.append(cleaned_landing_page_url)
    else:
        cleaned_landing_page_url = None

    document_type = first_string(record.get("document_types")) or "Thesis"
    ...
    raw_record = {
        ...
        "in_archive": record.get("in_archive"),
        "discoverable": record.get("discoverable"),
        ...
        "type": document_type,
        "original_publication_date": record.get("publication_date"),
    ...
    repository_identifier = str(handle).strip() if handle else external_id
    ...
    return NormalizedPublication(
        ...
        keywords=subjects,
        subjects=subjects,
        language=first_string(record.get("languages")),
        doi=None,
        orcid=None,
        issn=None,
        isbn=None,
        ...
        harvested_at=parse_datetime(record.get("harvested_at")),
        updated_at=parse_datetime(record.get("last_modified")),
        quality_score=0.0,
        is_deleted=bool(record.get("withdrawn", False)),
...
def load_json_records(
    ...
    if not json_file.exists():
        raise FileNotFoundError(f"JSON file was not found: {json_file}")
    ...
    if not isinstance(payload, list):
        raise ValueError("The AAU JSON file must contain a JSON array.")

    records = [record for record in payload if isinstance(record, dict)]

    skipped_count = len(payload) - len(records)
...
async def import_records(
    ...
    raw_records = load_json_records(json_file)

    mapped_records: list[NormalizedPublication] = []

    mapping_errors: list[dict[str, Any]] = []

    for index, raw_record in enumerate(raw_records):
        try:
            mapped_record = map_aau_record(raw_record)
    ...
    error_file = json_file.with_suffix(".import-errors.json")
    ...
    if mapping_errors:
        ...
        print(f"Mapping errors written to: {error_file}")
    elif error_file.exists():
        error_file.unlink()

    if dry_run:
        print("Dry run completed. Nothing was saved.")
        return
    ...
    context = HarvestPersistenceContext(
        ...
        repository_base_url=("https://etd.aau.edu.et"),
        connector_code=("aau-dspace-discovery"),
    )
    ...
    async with SessionLocal() as session:
        ...
        for start in range(
            ...
            batch = mapped_records[start : start + batch_size]
            ...
            try:
                ...
                LOGGER.exception(
                    "Batch persistence failed at records %s-%s.",
                    start,
                    start + len(batch) - 1,
                )

                totals["failed_count"] += len(batch)
            ...
            print(f"Processed {processed}/{len(mapped_records)}")
    ...
    summary_file = json_file.with_suffix(".import-summary.json")
...
def main() -> None:
    try:
        ...
        LOGGER.exception("AAU JSON import failed.")
        raise
