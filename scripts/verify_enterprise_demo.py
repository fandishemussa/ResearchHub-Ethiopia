    ...
    async with SessionLocal() as session:
        ...
        publication_count = int(
            await session.scalar(
                select(func.count(Publication.id)).where(Publication.source == "enterprise-demo")
            )
            or 0
        )
        checks = {
            "demo_university": bool(university and university.metadata_json.get("demo") is True),
            "disabled_demo_source": bool(
                connector and connector.config.get("demo") is True and not connector.enabled
            ),
