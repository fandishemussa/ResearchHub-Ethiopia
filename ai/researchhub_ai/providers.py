...
class AIProvider(Protocol):
    ...
    async def generate_text(self, prompt: str, *, model: str | None = None) -> TextGeneration: ...
    async def generate_chat_response(self, messages: list[dict[str, str]], *, model: str | None = None) -> TextGeneration: ...
...
class OllamaAIProvider:
    ...
    async def generate_text(self, prompt: str, *, model: str | None = None) -> TextGeneration:
        payload = {"model": model or self.default_model, "prompt": prompt, "stream": False, "options": {"temperature": 0.1}}
        data = await self._post("/api/generate", payload)
        return TextGeneration(text=str(data.get("response", "")), model=str(data.get("model", payload["model"])), provider=self.name, prompt_tokens=int(data.get("prompt_eval_count", 0)), completion_tokens=int(data.get("eval_count", 0)))

    async def generate_chat_response(self, messages: list[dict[str, str]], *, model: str | None = None) -> TextGeneration:
        payload = {"model": model or self.default_model, "messages": messages, "stream": False, "options": {"temperature": 0.1}}
        data = await self._post("/api/chat", payload)
        message = data.get("message") if isinstance(data.get("message"), dict) else {}
        return TextGeneration(text=str(message.get("content", "")), model=str(data.get("model", payload["model"])), provider=self.name, prompt_tokens=int(data.get("prompt_eval_count", 0)), completion_tokens=int(data.get("eval_count", 0)))
...
class OpenAICompatibleProvider:
    ...
    async def generate_text(self, prompt: str, *, model: str | None = None) -> TextGeneration:
        return await self.generate_chat_response([{"role": "user", "content": prompt}], model=model)

    async def generate_chat_response(self, messages: list[dict[str, str]], *, model: str | None = None) -> TextGeneration:
        async with httpx.AsyncClient(
            timeout=_http_timeout(self.timeout), headers=self.headers
        ) as client:
            response = await client.post(f"{self.base_url}/chat/completions", json={"model": model or self.default_model, "messages": messages, "temperature": 0.1})
            response.raise_for_status()
            data = response.json()
        usage = data.get("usage", {})
        return TextGeneration(text=str(data["choices"][0]["message"]["content"]), model=str(data.get("model", model or self.default_model)), provider=self.name, prompt_tokens=int(usage.get("prompt_tokens", 0)), completion_tokens=int(usage.get("completion_tokens", 0)))

    async def generate_embeddings(self, texts: list[str]) -> list[list[float]]:
        async with httpx.AsyncClient(
            timeout=_http_timeout(self.timeout), headers=self.headers
        ) as client:
            response = await client.post(f"{self.base_url}/embeddings", json={"model": self.default_model, "input": texts})
            response.raise_for_status()
            data = response.json()
...
def create_ai_provider(
    ...
    *,
    ...
    if normalized == "ollama":
        ...
        return OllamaAIProvider(ollama_base_url, chat_model, timeout)
